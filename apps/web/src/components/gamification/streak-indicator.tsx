...
import { FlameIcon, SnowflakeIcon, CalendarIcon } from 'lucide-react';
              ...
              <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                <motion.div
                  ...
                  animate={{
                    width: `${
                      ((currentStreak -
                        (STREAK_MILESTONES.find((m) => m.days <= currentStreak)?.days || 0)) /
                        (nextMilestone.days -
                          (STREAK_MILESTONES.find((m) => m.days <= currentStreak)?.days || 0))) *
                      100
                    }%`,
...
export function StreakCalendar({
  ...
  const activeDatesSet = new Set(
    activeDays.map((d) => new Date(d).toDateString())
  );
...
export function StreakMilestoneList({
  currentStreak,
  className,
}: StreakMilestoneListProps) {
  return (
