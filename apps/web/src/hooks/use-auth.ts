...
export function useUser() {
    ...
    return {
        ...
        user: session.data?.user ? {
            ...
            emailAddresses: session.data.user.email ? [{ emailAddress: session.data.user.email }] : [],
            primaryEmailAddress: session.data.user.email ? { emailAddress: session.data.user.email } : null,
