...
import { Check, X, ArrowRight, Command, Lightning, Brain, Shield, Clock, Eye, Sparkle } from '@phosphor-icons/react';
...
function PricingTier({
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        <div className="font-mono text-xs text-zinc-500 tracking-widest uppercase mb-2">{name}</div>
        ...
        <ul className="space-y-4 mb-8">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-3 text-sm text-zinc-300">
              <Check className="w-5 h-5 text-cyan-400 mt-0.5 flex-shrink-0" weight="bold" />
              {feature}
            </li>
          ))}
          {limitations?.map((limitation, i) => (
            <li key={`limit-${i}`} className="flex items-start gap-3 text-sm text-zinc-600">
              <X className="w-5 h-5 text-zinc-700 mt-0.5 flex-shrink-0" weight="bold" />
              {limitation}
            </li>
          ))}
        </ul>
...
function FAQItem({ question, answer, index }: { question: string; answer: string; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <h4 className="text-white font-medium group-hover:text-cyan-400 transition-colors">{question}</h4>
        <motion.span
...
export default function Pricing2Page() {
  ...
  const comparisonFeatures = [
    ...
    { feature: "AI Models", starter: "2 models", pro: "All models", ultra: "All + experimental" },
    ...
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 px-6 py-6 bg-black/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link href="/v2#features" className="text-sm text-zinc-400 hover:text-white transition-colors font-mono tracking-wide">
              Features
            </Link>
            <Link href="/v2#models" className="text-sm text-zinc-400 hover:text-white transition-colors font-mono tracking-wide">
              Models
            </Link>
            <Link href="/pricing2" className="text-sm text-white transition-colors font-mono tracking-wide">
              Pricing
            </Link>
            <Link
              href="/signup"
      ...
      <section className="pt-40 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            Access GPT-4, Claude, and Gemini for less than what you&apos;d pay for individual subscriptions.
            All with unified memory and intelligent routing.
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            >
              ...
              <p className="text-zinc-500 text-sm">Individual AI subscriptions cost $20-60 each. Get all of them unified for less.</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-zinc-500 text-sm">No more re-explaining context. Your memory follows you across models and sessions.</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-zinc-500 text-sm">We never train on your data. Export or delete your memory anytime.</p>
            </motion.div>
          </div>
        </div>
      </CinematicSection>
      ...
      <section className="py-24 px-6 bg-zinc-950/50">
        <div className="max-w-5xl mx-auto">
          <CinematicSection className="mb-12 text-center">
            <span className="font-mono text-xs text-cyan-400 tracking-widest uppercase mb-4 block">Compare Plans</span>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="py-4 text-left text-sm font-mono text-zinc-500 tracking-wide">Features</th>
                  <th className="py-4 text-center text-sm font-mono text-zinc-500 tracking-wide w-32">Starter</th>
                  <th className="py-4 text-center text-sm font-mono text-cyan-400 tracking-wide w-32 bg-cyan-500/5">Pro</th>
                  <th className="py-4 text-center text-sm font-mono text-zinc-500 tracking-wide w-32">Ultra</th>
                </tr>
              </thead>
              <tbody>
