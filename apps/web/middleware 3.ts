...
export async function middleware(request: NextRequest) {
    ...
    // cookie no longer exists, so the old `hasSession` check would 401/redirect
    // every authenticated request. Let requests pass through until Day 4 wires
    // WorkOS authkitMiddleware; route-level guards stay disabled in the stub.
    // const sessionCookie = request.cookies.get('better-auth.session_token');
    // const hasSession = !!sessionCookie?.value;

    // API routes: pass through (WorkOS authkitMiddleware will gate in Day 4)
