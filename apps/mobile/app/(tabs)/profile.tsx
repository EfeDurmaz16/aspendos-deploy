...
export default function ProfileScreen() {
    ...
    const MenuSection = ({ title, children }: { title: string; children: React.ReactNode }) => (
        <View className="mb-6">
            <Text className={`text-xs font-medium ${subtextColor} mb-2 px-4`}>{title}</Text>
            <View className={`${cardBg} rounded-xl overflow-hidden mx-4`}>{children}</View>
        </View>
    );
    ...
    return (
        <SafeAreaView className={`flex-1 ${bgColor}`} edges={['top']}>
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {/* Version */}
                <Text className={`text-center ${subtextColor} text-xs mb-8`}>YULA v0.1.0</Text>
            </ScrollView>
        </SafeAreaView>
    );
}
