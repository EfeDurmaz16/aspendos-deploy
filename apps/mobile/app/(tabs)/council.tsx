...
import { Send, BookOpen, Lightbulb, BarChart3, Sparkles, Check } from 'lucide-react-native';
            ...
            >
                ...
                <View className="flex-row items-center mb-2">
                    ...
                    <View className="flex-1">
                        <Text className={`font-semibold text-sm ${textColor}`}>{persona.name}</Text>
                        <Text className={`text-xs ${subtextColor}`}>{persona.description}</Text>
                    </View>
                    {isSelected && (
                        <View
                ...
                <View className="min-h-[80px]">
                    {response?.isStreaming ? (
                        ...
                        <>
                            <Text className={`text-sm ${textColor} leading-5`} numberOfLines={4}>
                                {response.content}
                            </Text>
                            {response.latencyMs && (
                        ...
                        <Text className={`text-sm ${subtextColor}`}>Waiting for query...</Text>
                    )}
                </View>
            </TouchableOpacity>
        );
                    ...
                    {selectedPersona && (
                        <View className={`p-4 rounded-xl ${cardBg} border ${borderColor} mb-4`}>
                            <Text className={`font-semibold ${textColor} mb-2`}>
                                {personas.find((p) => p.id === selectedPersona)?.name}'s Full
                                Response
                            </Text>
                        ...
                        >
                            {isLoading ? (
                                ...
                                <Send
                                    size={18}
                                    color={
                                        query.trim() ? '#FFFFFF' : isDark ? '#737373' : '#A3A3A3'
                                    }
                                    strokeWidth={2}
                                />
                            )}
