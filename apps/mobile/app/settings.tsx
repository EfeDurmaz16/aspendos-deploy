...
export default function SettingsScreen() {
    ...
    const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
        <View className="mb-6">
            ...
            <View className={`${cardBg} mx-4 rounded-xl overflow-hidden`}>{children}</View>
        </View>
    );
        ...
        >
            ...
            >
                <Icon
                    size={18}
                    color={
                        isDestructive ? '#EF4444' : iconColor || (isDark ? '#A3A3A3' : '#525252')
                    }
                />
            </View>
            <View className="flex-1">
                <Text className={`${isDestructive ? 'text-red-500' : textColor}`}>{label}</Text>
                {description && (
                    <Text className={`text-sm ${subtextColor} mt-0.5`}>{description}</Text>
                )}
            </View>
            {onToggle !== undefined && (
                <Switch
            ...
            {showChevron && <ChevronRight size={18} color={isDark ? '#525252' : '#A3A3A3'} />}
        </TouchableOpacity>
    );
