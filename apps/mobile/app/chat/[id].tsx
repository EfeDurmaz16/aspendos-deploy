...
export default function ChatScreen() {
    ...
    const handleSend = async () => {
        ...
        setMessages((prev) =>
            prev.map((m) =>
                m.id === assistantId ? { ...m, isStreaming: false, model: 'claude-3-5-sonnet' } : m
            )
        );
        setIsLoading(false);
    };
                ...
                <View className={`mb-4 ${isUser ? 'items-end' : 'items-start'}`}>
                    ...
                    >
                        <Text
                            className={`text-base leading-6 ${
                                isUser ? 'text-white dark:text-gray-900' : textColor
                            }`}
                        >
                            {message.content}
                            {message.isStreaming && <Text className="text-blue-500"> ▋</Text>}
                        </Text>
                    </View>
                    ...
                    {!isUser && !message.isStreaming && (
                        <View className="flex-row items-center mt-1 ml-1">
                            {message.model && (
                                <Text className={`text-xs ${subtextColor}`}>{message.model}</Text>
                            )}
                            <TouchableOpacity
