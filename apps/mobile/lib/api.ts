...
export const chats = {
    list: (archived = false) => request<Chat[]>(`/chats?archived=${archived}`),

    get: (id: string) => request<{ chat: Chat; messages: Message[] }>(`/chats/${id}`),
...
export const pac = {
    ...
    get: (id: string) => request<Reminder>(`/pac/reminders/${id}`),

    dismiss: (id: string) => request<void>(`/pac/reminders/${id}/dismiss`, { method: 'POST' }),
    ...
    getSettings: () => request<any>('/pac/settings'),
...
export const council = {
    ...
    history: () => request<CouncilSession[]>('/council/history'),
};
...
export const memory = {
    ...
    search: (query: string) => request<Memory[]>(`/memory/search?q=${encodeURIComponent(query)}`),
};
