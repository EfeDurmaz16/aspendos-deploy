...
app.post(
    '/sessions',
    requireApiKeyPermission('council:write'),
    validateBody(createCouncilSessionSchema),
    async (c) => {
 ...
 */
app.get('/sessions', requireApiKeyPermission('council:read'), async (c) => {
    const userId = c.get('userId')!;
...
app.get(
    '/sessions/:id',
    requireApiKeyPermission('council:read'),
    validateParams(sessionIdParamSchema),
    async (c) => {
        const userId = c.get('userId')!;
        const { id: sessionId } = c.get('validatedParams') as { id: string };
 ...
 */
app.get('/personas', requireApiKeyPermission('council:read'), async (c) => {
    return c.json({
        personas: Object.entries(councilService.COUNCIL_PERSONAS).map(([key, def]) => ({
            id: key,
            name: def.name,
 ...
 */
app.get('/stats', requireApiKeyPermission('council:read'), async (c) => {
    const userId = c.get('userId')!;
