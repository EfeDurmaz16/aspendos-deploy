...
describe('usage-ledger', () => {
    ...
    describe('calculateCostFromTokens', () => {
        it('should calculate cost for groq/llama-3.3-70b', () => {
            const cost = calculateCostFromTokens('groq/llama-3.3-70b', 1_000_000, 1_000_000);
            expect(cost).toBeCloseTo(0.59 + 0.79, 6);
        });

        it('should calculate cost for anthropic/claude-sonnet', () => {
            const cost = calculateCostFromTokens('anthropic/claude-sonnet', 500_000, 200_000);
            expect(cost).toBeCloseTo((500_000 / 1_000_000) * 3.0 + (200_000 / 1_000_000) * 15.0, 6);
        });

        it('should calculate cost for openai/gpt-4o-mini', () => {
            const cost = calculateCostFromTokens('openai/gpt-4o-mini', 100_000, 50_000);
            expect(cost).toBeCloseTo((100_000 / 1_000_000) * 0.15 + (50_000 / 1_000_000) * 0.6, 6);
        });
        ...
        it('should handle zero tokens', () => {
            const cost = calculateCostFromTokens('groq/llama-3.1-8b', 0, 0);
            expect(cost).toBe(0);
        });
    ...
    describe('getUserCosts', () => {
        ...
        it('should group costs by model', () => {
            const costs = usageLedger.getUserCosts('user1');
            expect(costs.byModel['groq/llama-3.3-70b'].cost).toBeCloseTo(0.001, 6);
            expect(costs.byModel['anthropic/claude-sonnet'].cost).toBeCloseTo(0.02, 6);
        });

        it('should track input and output tokens by model', () => {
            const costs = usageLedger.getUserCosts('user1');
            expect(costs.byModel['groq/llama-3.3-70b'].inputTokens).toBe(1000);
            expect(costs.byModel['groq/llama-3.3-70b'].outputTokens).toBe(500);
        });
        ...
        it('should track request count by model', () => {
            ...
            const costs = usageLedger.getUserCosts('user1');
            expect(costs.byModel['groq/llama-3.3-70b'].requestCount).toBe(2);
        });
    ...
    describe('getCostBreakdown', () => {
        it('should return comprehensive breakdown', () => {
            ...
            expect(breakdown.byModel['groq/llama-3.3-70b']).toBeDefined();
    ...
    describe('getTopSpenders', () => {
        ...
        it('should identify most used model', () => {
            ...
            const topSpenders = usageLedger.getTopSpenders();
            const user1 = topSpenders.find((s) => s.userId === 'user1');
            expect(user1?.mostUsedModel).toBe('groq/llama-3.3-70b');
        });
    ...
    describe('edge cases', () => {
        it('should handle very large token counts', () => {
            const cost = calculateCostFromTokens('anthropic/claude-sonnet', 10_000_000, 5_000_000);
            expect(cost).toBeGreaterThan(0);
        ...
        it('should handle multiple entries for same user and model', () => {
            ...
            const costs = usageLedger.getUserCosts('user1');
            expect(costs.byModel['groq/llama-3.3-70b'].requestCount).toBe(5);
