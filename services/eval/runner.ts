import { runGaiaTask, computeScore, type GaiaTask, type GaiaResult } from './gaia';
...
const GAIA_LEVEL1_SAMPLE: GaiaTask[] = [
    ...
    { id: 'g3', level: 1, question: 'Who wrote Romeo and Juliet?', expected_answer: 'William Shakespeare' },
    { id: 'g4', level: 1, question: 'What is the chemical symbol for gold?', expected_answer: 'Au' },
    ...
    { id: 'g8', level: 1, question: 'Who painted the Mona Lisa?', expected_answer: 'Leonardo da Vinci' },
    { id: 'g9', level: 1, question: 'What is the largest planet in our solar system?', expected_answer: 'Jupiter' },
    { id: 'g10', level: 1, question: 'What is the boiling point of water in Celsius?', expected_answer: '100' },
];
...
async function main() {
    ...
    for (let i = 0; i < GAIA_LEVEL1_SAMPLE.length; i += concurrency) {
        ...
        const batchResults = await Promise.all(
            batch.map((task) => runGaiaTask(task, model)),
        );
    ...
    console.log(`\n=== Results ===`);
    console.log(`Accuracy: ${(score.accuracy * 100).toFixed(1)}% (${score.correct}/${score.total})`);
